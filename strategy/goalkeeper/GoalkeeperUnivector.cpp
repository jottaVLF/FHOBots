#include "GoalkeeperUnivector.hpp"
#include "../../Global.hpp"
...
namespace
{
    const double OBSTACLE_INFLUENCE  = 140.0;
    const double OBSTACLE_REPULSION  = 2.8;
    const double OBSTACLE_TANGENTIAL = 1.6;
    const double TARGET_ATTRACTION   = 1.0;
    const double LOOKAHEAD           = 44.0;
    const double OBSTACLE_RADIUS     = 26.0;
    const double PREDICTION_FRAMES   = 1.5;
    const double PREDICTION_MAX_PX   = 22.0;
    ...
    UnivectorField& goalkeeperField()
    {
    ...
    std::vector<UnivectorObstacle> collectObstacles(const Robot* robot)
    {
        ...
        if(robot != &Global::deffender && isFiniteVector(Global::deffender.getPosition()))
            obstacles.push_back(UnivectorObstacle(Global::deffender.getPosition(), OBSTACLE_RADIUS));
...
bool GoalkeeperUnivector::applyNavigation(Robot* robot, const Vector2D& target)
{
    ...
    const UnivectorOutput output = goalkeeperField().compute(input);

    if(!output.valid || !isFiniteVector(output.waypoint))
        return false;

    // Controle proporcional direto — evita oscilação do kd=260 do goleiro em curvas
    const int base = std::max(50, std::min(180, (int)robot->getBasePwmValue()));
