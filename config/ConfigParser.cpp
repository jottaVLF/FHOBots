...
ConfigParser::ConfigParser(){
    ...
    if(!this->inputConfig.is_open()){
        std::cerr << "Error: could not open config/appConfig.json. Run the program from the repository root or restore the config file." <<std::endl;
        exit(1);
    }
...
Token * ConfigParser::getToken(){
    ...
    if(buffer.empty() || this->inputConfig.fail())
