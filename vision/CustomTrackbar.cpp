...
void loadColors(const std::string pathFile, cv::Scalar& high, cv::Scalar& low)
{
    ...
    std::cerr << "Error: HSV calibration file '" << pathFile << "' does not exist. Trackbar values will start at zero." << std::endl;
}
...
void saveColors(const std::string pathFile, cv::Scalar high, cv::Scalar low)
{
    ...
    if(file.is_open())
    {
        ...
        std::cerr << "Error: could not open HSV calibration file '" << pathFile << "' for writing.\n\n";
}
